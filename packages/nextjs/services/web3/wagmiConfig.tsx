...
const { targetNetworks, liveDeploy } = scaffoldConfig;

const activeTargetNetworks = liveDeploy
  ? targetNetworks.filter((network: Chain) => network.id !== hardhat.id)
  : targetNetworks;

// We always want to have mainnet enabled (ENS resolution, ETH price, etc). But only once.
const chainsWithMainnet = activeTargetNetworks.find((network: Chain) => network.id === 1)
